...
SSH 进程与本地回环监听决定隧道的 SSH/TCP 存活状态；可选的本机 HTTP 地址只负责单独报告转发后应用是否就绪。HTTP 超时或返回 `5xx` 时应用会显示“未就绪/降级”，但不会结束健康的 SSH 进程，也不会消耗 SSH 重试额度。如果已配置的反向代理目标对应这条隧道，Local Ops 还会自动检查包含访问路径的完整 `.localhost` 地址，并分别显示 SSH 链路、应用就绪状态和“域名入口：已就绪 / 未就绪”。完整入口来自现有反向代理配置，不需要在 SSH 隧道中重复填写。
...
从菜单栏停止服务或隧道时，现在必须来自可信的真实点击，并经过明确确认。控制 API 会拒绝未经确认的菜单栏停止请求，记录事件名、动作 ID、调用路径和确认结果；如果检测到未经确认的菜单栏停止，之前已记忆的隧道仍会保留在恢复集合中。
